...
function generateNaturalLanguageDescription(data) {
  try {
    ...
    var age = safe("Age");
    var genderRaw = safe("Gender");
    var gender = "person";

    if (age === "Teenage" || age === "Pre-adolescent Child") {
      gender = genderRaw.toLowerCase().indexOf("male") !== -1 ? "boy" : "girl";
    } else {
      gender = genderRaw.toLowerCase().indexOf("male") !== -1 ? "man" : "woman";
    }

    var heritage = safe("Visual Heritage");
    var build = combo("Build Intensity", "Build");
    var height = safe("Height");
    var skin = combo("Skin Tone Intensity", "Skin Tone");
    var skinTexture = combo("Skin Texture Visibility", "Skin Texture");
    var headShape = safe("Head Shape");
    var eyeColor = combo("Eye Color Intensity", "Eye Color");
    var hairColor = combo("Hair Color Tone", "Hair Color");
    var hairTexture = combo("Hair Quality", "Hair Texture");
    var hairLength = safe("Hair Length");

    var desc = "An " + age.toLowerCase() + " " + gender + " of " + heritage.toLowerCase() +
      " with a " + build.toLowerCase() + " build, " + height.toLowerCase() + " height, and " +
      skin.toLowerCase() + " skin with " + skinTexture.toLowerCase() + " texture. " +
      "They have a " + headShape.toLowerCase() + " head shape, " + eyeColor.toLowerCase() +
      " eyes, and " + hairLength.toLowerCase() + " " + hairTexture.toLowerCase() +
      " hair colored " + hairColor.toLowerCase() + ".";

    return refineDescriptionStyle(desc);
  } catch (e) {
    return "[Generation failure: " + (e.message || e) + "]";
  }
}
